export const filterPapers = (papers, filters) => {
  return papers.filter(paper => {
    ...
    if (filters.titleSearch && filters.titleSearch.trim()) {
      const titleSearchTerm = filters.titleSearch.toLowerCase().trim();
      if (!paper.title.toLowerCase().includes(titleSearchTerm)) {
        return false;
      }
    }

    // Filter by abstract
    if (filters.abstractSearch && filters.abstractSearch.trim()) {
      const abstractSearchTerm = filters.abstractSearch.toLowerCase().trim();
      if (!paper.abstract.toLowerCase().includes(abstractSearchTerm)) {
        return false;
      }
    }
